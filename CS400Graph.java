...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    PriorityQueue<Path> queue = new PriorityQueue<Path>(); // the priority queue to hold the
                                                           // discovered path and its frontiers
    LinkedList<Vertex> verticeTraveled = new LinkedList<>(); // keeping track of vertice explored
    Vertex startVertex = vertices.get(start);
    Path startPath = new Path(startVertex);
    queue.add(startPath);
    verticeTraveled.add(startVertex);
    while (!queue.isEmpty()) {
//      Path shortestPath = queue.remove(); // get the shortest Path
      Path shortestPath = queue.poll(); // get the shortest Path
      if (shortestPath.end.equals(vertices.get(end))) {
        return shortestPath;
      }
      for (Edge edge : shortestPath.end.edgesLeaving) {
        boolean isVisited = false;
        for (Vertex vertex : verticeTraveled) { // check to see if the neighboring node has been
                                                // visited
        ...
        if (!isVisited) {
          Path newPath = new Path(shortestPath); // if not visited, then add this to the path.
          newPath.extend(edge);
          queue.add(newPath);
          verticeTraveled.add(edge.target);
        }
      }
    }
    throw new NoSuchElementException("there is not a path found."); // throw this when no path between
                                                                  // the 2 given points were found
    // Note for myself: Review https://mediaspace.wisc.edu/media/1_y9jcsj3a
  }
